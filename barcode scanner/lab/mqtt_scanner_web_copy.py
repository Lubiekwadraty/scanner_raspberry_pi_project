    ...
    while True:
        ...
        if(succes):
            ...
            if(i > 2):
                ...
                plt.clf()
                plt.plot(hist2)
                plt.savefig("stream/image3.jpeg")
                compared = cv2.compareHist(hist1, hist2, cv2.HISTCMP_CHISQR)
                time_compare += time.time() - t
   
                if(compared > 0.1):
                    print("difference", compared, len(hist1)) 
            else:
                frames.append(img)
            i += 1
            time_compare_counter += 1
            cv2.imwrite("stream/image1.jpeg", img)
            cv2.imwrite("stream/image2.jpeg", simillarity)

            # creating visualisation of histogram
            
            # img = cv2.imread('stream/image1.jpeg', cv2.IMREAD_GRAYSCALE)
        ...
        print(time_compare/time_compare_counter)
        # print(focus)
